...
struct cityValidator: ValidatorConvertible {
    func validated(_ value: String) throws -> String {
        guard value.count >= 3 else {
            throw ValidationError("City must contain more than three characters" )
        }
        guard value.count < 18 else {
            throw ValidationError("City shoudn't conain more than 18 characters" )
        }
        
        do {
            if try NSRegularExpression(pattern: "^[a-z]{1,18}$",  options: .caseInsensitive).firstMatch(in: value, options: [], range: NSRange(location: 0, length: value.count)) == nil {
                throw ValidationError("Invalid City, city should not contain whitespaces, numbers or special characters")
            }
        } catch {
            throw ValidationError("Invalid City, city should not contain whitespaces,  or special characters")
        }
        return value
    }
}
...
struct phoneValidator:ValidatorConvertible {
    func validated(_ value: String) throws -> String {
        guard value.count > 9 else {throw ValidationError("Phone is required")}
       // guard let age = Int(value) else {throw ValidationError("Phone must be a number!")}
        guard value.count < 15 else {throw ValidationError("Invalid phone number!")}
        //guard age >= 18 else {throw ValidationError("You have to be over 18 years old to user our app :)")}
        return value
    }
}
...
struct pincodeValidator:ValidatorConvertible {
    func validated(_ value: String) throws -> String {
        guard value.count > 4 else {throw ValidationError("Pincode is required")}
        return value
    }
}
